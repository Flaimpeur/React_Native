...
export const writefile = async (data) => {
    ...
    try {
        ...
        const fileInfo = await FileSystem.getInfoAsync(FileSystem.documentDirectory + 'text.txt')
        if (fileInfo.exists) {
            //Lire le fichier, et recuperer les donnees existantes
            const fileContent = await FileSystem.readAsStringAsync(FileSystem.documentDirectory + 'text.txt')
            ...
            // ensuite tu ...
            await FileSystem.writeAsStringAsync(FileSystem.documentDirectory + 'text.txt', newFile)
            console.log(newFile)
        } else {
            await FileSystem.writeAsStringAsync(FileSystem.documentDirectory + 'text.txt', data)
            console.log(data)
        }
    } catch (error) {
